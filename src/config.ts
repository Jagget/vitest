...
function enhanceProject(project: any, stencilConfig?: StencilConfig): any {
  ...
  if (enhanced.test) {
    ...
    if (environment === 'jsdom') {
      if (!setupFilesArray.includes('@stenciljs/vitest/jsdom-setup')) {
        enhanced.test.setupFiles = ['@stenciljs/vitest/jsdom-setup', ...setupFilesArray];
      }
    }

    // Auto-inject happy-dom-setup for happy-dom environment
    if (environment === 'happy-dom') {
      if (!setupFilesArray.includes('@stenciljs/vitest/happy-dom-setup')) {
        enhanced.test.setupFiles = ['@stenciljs/vitest/happy-dom-setup', ...setupFilesArray];
      }
    }
